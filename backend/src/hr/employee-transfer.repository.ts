...
export const HR_TRANSFER_TYPE = 'HR_TRANSFER';

@Injectable()
export class EmployeeTransferRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.employeeTransfer.findFirst({ where: { id, tenantId } });
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.employeeTransfer.create({
      data: {
        ...
        transferType: input.transferType as string,
        effectiveDate: input.effectiveDate as Date,
        newOrganizationId: input.newOrganizationId as string | undefined,
        newDepartmentId: input.newDepartmentId as string | undefined,
        newPositionId: input.newPositionId as string | undefined,
        newStaffingPositionId: input.newStaffingPositionId as string | undefined,
        newBranchId: input.newBranchId as string | undefined,
        newManagerEmploymentId: input.newManagerEmploymentId as string | undefined,
        newLocationId: input.newLocationId as string | undefined,
        newFte: input.newFte !== undefined ? (input.newFte as number).toString() : undefined,
        newWorkScheduleId: input.newWorkScheduleId as string | undefined,
