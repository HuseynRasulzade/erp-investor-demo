...
export const HR_TERMINATION_TYPE = 'HR_TERMINATION';

@Injectable()
export class TerminationRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.terminationDocument.findFirst({ where: { id, tenantId } });
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.terminationDocument.create({
      data: {
        ...
        terminationDate: input.terminationDate as Date,
        lastWorkingDate: input.lastWorkingDate as Date | undefined,
        terminationReason: input.terminationReason as string,
        legalReference: input.legalReference as string | undefined,
        noticeDate: input.noticeDate as Date | undefined,
