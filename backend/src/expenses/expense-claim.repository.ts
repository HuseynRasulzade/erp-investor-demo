  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.expenseClaim.create({
      data: {
        ...
        expensePeriodStart: input.expensePeriodStart as Date,
        expensePeriodEnd: input.expensePeriodEnd as Date,
        currencyId: input.currencyId as string,
        responsibleManagerId: input.responsibleManagerId as string | undefined,
