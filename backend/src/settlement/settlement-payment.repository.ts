...
export const SETTLEMENT_PAYMENT_TYPE = 'SETTLEMENT_PAYMENT';

@Injectable()
export class SettlementPaymentRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.settlementPayment.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.settlementPayment.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.settlementPayment.create({
      data: {
        tenantId,
        ...
        exchangeRate: input.exchangeRate != null ? (input.exchangeRate as number).toString() : undefined,
