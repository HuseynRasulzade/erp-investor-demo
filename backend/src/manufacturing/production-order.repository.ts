...
export const PRODUCTION_ORDER_TYPE = 'PRODUCTION_ORDER';

@Injectable()
export class ProductionOrderRepository implements DocumentRepositoryAdapter {
  readonly documentType = PRODUCTION_ORDER_TYPE;

  constructor(private readonly prisma: PrismaService) {}

  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    const client = tx ?? this.prisma;
    const row = await client.productionOrder.findFirst({ where: { id, tenantId } });
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.productionOrder.create({
      data: {
        ...
        productionStartDate: input.productionStartDate as Date | undefined,
        plannedEndDate: input.plannedEndDate as Date | undefined,
        productionType: (input.productionType as string) ?? 'MAKE_TO_STOCK',
        outputWarehouseId: input.outputWarehouseId as string,
        bomVersionId: input.bomVersionId as string,
        routingVersionId: input.routingVersionId as string | undefined,
        costCenterId: input.costCenterId as string | undefined,
        projectId: input.projectId as string | undefined,
        responsibleUserId: input.responsibleUserId as string | undefined,
        plannedOutputQuantity: (input.plannedOutputQuantity as number).toString(),
