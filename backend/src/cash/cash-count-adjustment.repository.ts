...
export const CASH_COUNT_ADJUSTMENT_TYPE = 'CASH_COUNT_ADJUSTMENT';

@Injectable()
export class CashCountAdjustmentRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.cashCountAdjustment.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.cashCountAdjustment.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.cashCountAdjustment.create({
      data: {
        ...
        cashDeskId: input.cashDeskId as string,
        physicalCountId: input.physicalCountId as string | undefined,
        adjustmentType: input.adjustmentType as string,
  ...
  private toBaseFields(row: any): BaseDocumentFields {
    return {
      ...
      exchangeRate: null,
      description: row.adjustmentType,
