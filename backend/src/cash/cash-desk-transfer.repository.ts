...
export class CashDeskTransferRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.cashDeskTransfer.findFirst({ where: { id, tenantId } });
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.cashDeskTransfer.create({
      data: {
        ...
        sourceCashDeskId: input.sourceCashDeskId as string,
        destinationCashDeskId: input.destinationCashDeskId as string,
