...
export const INTERNAL_BANK_TRANSFER_TYPE = 'INTERNAL_BANK_TRANSFER';

@Injectable()
export class InternalBankTransferRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.internalBankTransfer.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.internalBankTransfer.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.internalBankTransfer.create({
      data: {
        ...
        feeAmount: input.feeAmount != null ? (input.feeAmount as number).toString() : undefined,
        transferMode: (input.transferMode as string) ?? 'INSTANT',
