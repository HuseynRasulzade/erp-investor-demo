import { Injectable } from '@nestjs/common';
import { PrismaService, PrismaTransactionClient } from '../prisma/prisma.service';
import { DocumentRepositoryAdapter, DocumentStatusPatch } from '../document-framework/document-repository.interface';
import { BaseDocumentFields } from '../document-framework/base-document';

export const FX_CONVERSION_TYPE = 'FX_CONVERSION';

@Injectable()
export class FXConversionRepository implements DocumentRepositoryAdapter {
  readonly documentType = FX_CONVERSION_TYPE;

  constructor(private readonly prisma: PrismaService) {}

  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    const client = tx ?? this.prisma;
    const row = await client.fXConversion.findFirst({ where: { id, tenantId } });
    return row ? this.toBaseFields(row) : null;
  }

  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.fXConversion.updateMany({
      where: { id, tenantId, version: expectedVersion },
      data: {
        ...(patch.status ? { status: patch.status } : {}),
        ...(patch.postingStatus ? { postingStatus: patch.postingStatus } : {}),
        ...(patch.postedAt !== undefined ? { postedAt: patch.postedAt } : {}),
        ...(patch.postedBy !== undefined ? { postedBy: patch.postedBy } : {}),
        ...(patch.cancelledAt !== undefined ? { cancelledAt: patch.cancelledAt } : {}),
        ...(patch.cancelledBy !== undefined ? { cancelledBy: patch.cancelledBy } : {}),
        version: { increment: 1 },
      },
    });
    return { updatedCount: result.count, newVersion: expectedVersion + 1 };
  }

  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.fXConversion.create({
      data: {
        tenantId,
        organizationId: input.organizationId as string,
        sourceBankAccountId: input.sourceBankAccountId as string,
        destinationBankAccountId: input.destinationBankAccountId as string,
        sourceCurrencyId: input.sourceCurrencyId as string,
        sourceAmount: (input.sourceAmount as number).toString(),
        destinationCurrencyId: input.destinationCurrencyId as string,
        destinationAmount: (input.destinationAmount as number).toString(),
        tradeRate: (input.tradeRate as number).toString(),
        officialRate: input.officialRate != null ? (input.officialRate as number).toString() : undefined,
        bankFeeAmount: input.bankFeeAmount != null ? (input.bankFeeAmount as number).toString() : undefined,
        bankReference: input.bankReference as string | undefined,
        number: input.number as string | undefined,
        conversionDate: (input.conversionDate as Date) ?? new Date(),
        createdBy,
        updatedBy: createdBy,
      },
    });
    return this.toBaseFields(row);
  }

  private toBaseFields(row: any): BaseDocumentFields {
    return {
      id: row.id,
      tenantId: row.tenantId,
      organizationId: row.organizationId,
      documentType: row.documentType,
      number: row.number,
      documentDate: row.conversionDate,
      postingDate: row.postingDate,
      status: row.status,
      postingStatus: row.postingStatus,
      currencyId: row.sourceCurrencyId,
      exchangeRate: row.tradeRate,
      description: null,
      createdAt: row.createdAt,
      createdBy: row.createdBy,
      updatedAt: row.updatedAt,
      updatedBy: row.updatedBy,
      postedAt: row.postedAt,
      postedBy: row.postedBy,
      cancelledAt: row.cancelledAt,
      cancelledBy: row.cancelledBy,
      deletionMark: row.deletionMark,
      version: row.version,
    };
  }
}
