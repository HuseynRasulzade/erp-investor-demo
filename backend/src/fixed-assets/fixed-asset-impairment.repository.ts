...
export const FIXED_ASSET_IMPAIRMENT_TYPE = 'FIXED_ASSET_IMPAIRMENT';

@Injectable()
export class FixedAssetImpairmentRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.fixedAssetImpairment.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.fixedAssetImpairment.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.fixedAssetImpairment.create({
      data: {
        ...
        carryingAmountBefore: (input.carryingAmountBefore as number).toString(),
        recoverableAmount: (input.recoverableAmount as number).toString(),
        impairmentAmount: (input.impairmentAmount as number).toString(),
        reason: input.reason as string | undefined,
        valuationSource: input.valuationSource as string | undefined,
        reversal: (input.reversal as boolean) ?? false,
