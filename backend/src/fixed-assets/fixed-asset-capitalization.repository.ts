...
export const FIXED_ASSET_CAPITALIZATION_TYPE = 'FIXED_ASSET_CAPITALIZATION';

@Injectable()
export class FixedAssetCapitalizationRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.fixedAssetCapitalization.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.fixedAssetCapitalization.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.fixedAssetCapitalization.create({
      data: {
        tenantId,
        organizationId: input.organizationId as string,
        cipProjectId: input.cipProjectId as string | undefined,
