...
export const FIXED_ASSET_REVALUATION_TYPE = 'FIXED_ASSET_REVALUATION';

@Injectable()
export class FixedAssetRevaluationRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.fixedAssetRevaluation.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.fixedAssetRevaluation.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.fixedAssetRevaluation.create({
      data: {
        ...
        oldCarryingAmount: (input.oldCarryingAmount as number).toString(),
        revaluedAmount: (input.revaluedAmount as number).toString(),
