...
export const FIXED_ASSET_MODERNIZATION_TYPE = 'FIXED_ASSET_MODERNIZATION';

@Injectable()
export class FixedAssetModernizationRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.fixedAssetModernization.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.fixedAssetModernization.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.fixedAssetModernization.create({
      data: {
        ...
        modernizationType: (input.modernizationType as string) ?? 'CAPITAL_IMPROVEMENT',
        sourceDocumentType: input.sourceDocumentType as string | undefined,
        ...
        newUsefulLifeMonths: input.newUsefulLifeMonths as number | undefined,
        newResidualValue: input.newResidualValue !== undefined ? (input.newResidualValue as number).toString() : undefined,
