...
export const FIXED_ASSET_TRANSFER_TYPE = 'FIXED_ASSET_TRANSFER';

@Injectable()
export class FixedAssetTransferRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.fixedAssetTransfer.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.fixedAssetTransfer.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.fixedAssetTransfer.create({
      data: {
        ...
        fromDepartmentId: input.fromDepartmentId as string | undefined,
        toDepartmentId: input.toDepartmentId as string | undefined,
        fromLocationId: input.fromLocationId as string | undefined,
        toLocationId: input.toLocationId as string | undefined,
        fromResponsiblePersonId: input.fromResponsiblePersonId as string | undefined,
        toResponsiblePersonId: input.toResponsiblePersonId as string | undefined,
