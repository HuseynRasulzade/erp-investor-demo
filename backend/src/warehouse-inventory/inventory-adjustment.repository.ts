...
export const INVENTORY_ADJUSTMENT_TYPE = 'INVENTORY_ADJUSTMENT';

@Injectable()
export class InventoryAdjustmentRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.inventoryAdjustment.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.inventoryAdjustment.updateMany({
