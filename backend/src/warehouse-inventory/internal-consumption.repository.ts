...
export const INTERNAL_CONSUMPTION_TYPE = 'INTERNAL_CONSUMPTION';

@Injectable()
export class InternalConsumptionRepository implements DocumentRepositoryAdapter {
  ...
  async findById(tenantId: string, id: string, tx?: PrismaTransactionClient): Promise<BaseDocumentFields | null> {
    ...
    const row = await client.internalConsumption.findFirst({ where: { id, tenantId } });
  ...
  async applyStatusPatch(tenantId: string, id: string, patch: DocumentStatusPatch, expectedVersion: number, tx: PrismaTransactionClient) {
    const result = await tx.internalConsumption.updateMany({
  ...
  async create(tenantId: string, input: Record<string, unknown>, createdBy: string, tx: PrismaTransactionClient): Promise<BaseDocumentFields> {
    const row = await tx.internalConsumption.create({
      data: {
        tenantId,
        organizationId: input.organizationId as string,
        warehouseId: input.warehouseId as string,
        departmentId: input.departmentId as string | undefined,
        operationType: (input.operationType as string) ?? 'OFFICE_CONSUMPTION',
